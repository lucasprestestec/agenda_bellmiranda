...
export function BookingFlow({ services: allServices, initialServiceSlug, layout = 'desktop' }) {
  ...
  function selectStaffCat(key) {
    ...
    if (group?.items[0]) setSvcSlug(group.items[0].slug);
  }
        ...
        <div style={{ maxWidth: '640px', margin: '0 auto' }}>
          <ConfirmationPanel
            ...
            actions={
              <>
                <Button variant="whatsapp" href={SITE.whatsappHref} iconLeft={<Icon name="message-circle" size={15} />}>Falar no WhatsApp</Button>
                <Button variant="secondary" onClick={() => { setDone(false); setStep('servico'); setConfirmation(null); setName(''); setPhone(''); setNote(''); }}>Agendar outro horário</Button>
              </>
            }
          />
