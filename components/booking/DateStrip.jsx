          ...
          return (
            <button key={d.value} disabled={d.disabled} onClick={()=>onChange && onChange(d.value)}
              style={{ flex:'0 0 auto', width:'62px', height: count!=null ? '90px' : '78px', display:'flex', flexDirection:'column',
                ...
                borderRadius:'var(--radius-md)', background: on ? 'var(--blush-400)' : 'var(--surface-card)',
                border:'1px solid '+(on ? 'var(--blush-400)' : 'var(--border-hairline)'),
                color: on ? 'var(--cocoa-800)' : 'var(--text-body)', opacity:d.disabled?.35:1,
