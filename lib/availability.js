...
async function busyIntervalsForDay(dateISO, excludeAppointmentId) {
...
// overlap an existing appointment/block on that day? Unlike the public
// listSlotsForDay, this has no "must be in the future" restriction and can
// exclude one appointment (so editing its own slot doesn't conflict with itself).
export async function isRangeFree({ dateISO, startTime, durationMin, excludeAppointmentId }) {
  const busy = await busyIntervalsForDay(dateISO, excludeAppointmentId);
