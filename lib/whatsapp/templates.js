...
export function cancellationTemplate({ appointment, serviceView }) {
  const text = [
    `Olá, ${appointment.clientName}!`,
    '',
...
export function teamCancellationTemplate({ appointment, serviceView }) {
  const lines = [
...
// previousDate/previousStartTime are set by the PATCH routes right before
// applying a reschedule — see prisma/schema.prisma for why they exist.
export function rescheduleTemplate({ appointment, serviceView }) {
  const text = [
    `Olá, ${appointment.clientName}!`,
    '',
    ...
    '',
    `Serviço: ${serviceView.name}`,
    `De: ${formatLong(appointment.previousDate)}, ${appointment.previousStartTime}`,
    `Para: ${formatLong(appointment.date)}, ${appointment.startTime}`,
    '',
    ...contactNotice(serviceView),
  ].join('\n');
  return { text };
...
export function teamRescheduleTemplate({ appointment, serviceView }) {
  const lines = [
